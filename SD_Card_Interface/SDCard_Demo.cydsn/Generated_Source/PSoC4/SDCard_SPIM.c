...
static void SDCard_SPIM_ScbModeStop(void)
{
    ...
    *******************************************************************************/
    void SDCard_SPIM_SetPins(uint32 mode, uint32 subMode, uint32 uartEnableMask)
    {
        ...
        {
            SDCard_SPIM_RX_SDA_MOSI_HSIOM_SEL_GPIO,
            SDCard_SPIM_TX_SCL_MISO_HSIOM_SEL_GPIO,
            0u,
            0u,
            0u,
            0u,
            ...
            (SDCard_SPIM_SCB_MODE_EZI2C == mode))
        {
            ...
            hsiomSel[SDCard_SPIM_RX_SDA_MOSI_PIN_INDEX] = SDCard_SPIM_RX_SDA_MOSI_HSIOM_SEL_I2C;
            pinsDm  [SDCard_SPIM_RX_SDA_MOSI_PIN_INDEX] = SDCard_SPIM_PIN_DM_OD_LO;
        #elif (SDCard_SPIM_RX_WAKE_SDA_MOSI_PIN)
            hsiomSel[SDCard_SPIM_RX_WAKE_SDA_MOSI_PIN_INDEX] = SDCard_SPIM_RX_WAKE_SDA_MOSI_HSIOM_SEL_I2C;
            pinsDm  [SDCard_SPIM_RX_WAKE_SDA_MOSI_PIN_INDEX] = SDCard_SPIM_PIN_DM_OD_LO;
        #else
        #endif /* (SDCard_SPIM_RX_SDA_MOSI_PIN) */
        
        #if (SDCard_SPIM_TX_SCL_MISO_PIN)
            hsiomSel[SDCard_SPIM_TX_SCL_MISO_PIN_INDEX] = SDCard_SPIM_TX_SCL_MISO_HSIOM_SEL_I2C;
            pinsDm  [SDCard_SPIM_TX_SCL_MISO_PIN_INDEX] = SDCard_SPIM_PIN_DM_OD_LO;
        ...
        {
        #if (SDCard_SPIM_RX_SDA_MOSI_PIN)
            hsiomSel[SDCard_SPIM_RX_SDA_MOSI_PIN_INDEX] = SDCard_SPIM_RX_SDA_MOSI_HSIOM_SEL_SPI;
        #elif (SDCard_SPIM_RX_WAKE_SDA_MOSI_PIN)
            hsiomSel[SDCard_SPIM_RX_WAKE_SDA_MOSI_PIN_INDEX] = SDCard_SPIM_RX_WAKE_SDA_MOSI_HSIOM_SEL_SPI;
        #else
        #endif /* (SDCard_SPIM_RX_SDA_MOSI_PIN) */
        
        #if (SDCard_SPIM_TX_SCL_MISO_PIN)
            hsiomSel[SDCard_SPIM_TX_SCL_MISO_PIN_INDEX] = SDCard_SPIM_TX_SCL_MISO_HSIOM_SEL_SPI;
        #endif /* (SDCard_SPIM_TX_SCL_MISO_PIN) */
        
        #if (SDCard_SPIM_CTS_SCLK_PIN)
            hsiomSel[SDCard_SPIM_CTS_SCLK_PIN_INDEX] = SDCard_SPIM_CTS_SCLK_HSIOM_SEL_SPI;
        #endif /* (SDCard_SPIM_CTS_SCLK_PIN) */

            if (SDCard_SPIM_SPI_SLAVE == subMode)
            {
                /* Slave */
                pinsDm[SDCard_SPIM_RX_SDA_MOSI_PIN_INDEX] = SDCard_SPIM_PIN_DM_DIG_HIZ;
                pinsDm[SDCard_SPIM_TX_SCL_MISO_PIN_INDEX] = SDCard_SPIM_PIN_DM_STRONG;
                pinsDm[SDCard_SPIM_CTS_SCLK_PIN_INDEX] = SDCard_SPIM_PIN_DM_DIG_HIZ;

            #if (SDCard_SPIM_RTS_SS0_PIN)
                /* Only SS0 is valid choice for Slave */
                hsiomSel[SDCard_SPIM_RTS_SS0_PIN_INDEX] = SDCard_SPIM_RTS_SS0_HSIOM_SEL_SPI;
                pinsDm  [SDCard_SPIM_RTS_SS0_PIN_INDEX] = SDCard_SPIM_PIN_DM_DIG_HIZ;
            #endif /* (SDCard_SPIM_RTS_SS0_PIN) */

            #if (SDCard_SPIM_TX_SCL_MISO_PIN)
                /* Disable input buffer */
                 pinsInBuf |= SDCard_SPIM_TX_SCL_MISO_PIN_MASK;
            #endif /* (SDCard_SPIM_TX_SCL_MISO_PIN) */
            }
            else 
            {
                /* (Master) */
                pinsDm[SDCard_SPIM_RX_SDA_MOSI_PIN_INDEX] = SDCard_SPIM_PIN_DM_STRONG;
                pinsDm[SDCard_SPIM_TX_SCL_MISO_PIN_INDEX] = SDCard_SPIM_PIN_DM_DIG_HIZ;
                pinsDm[SDCard_SPIM_CTS_SCLK_PIN_INDEX] = SDCard_SPIM_PIN_DM_STRONG;

            #if (SDCard_SPIM_RTS_SS0_PIN)
                hsiomSel [SDCard_SPIM_RTS_SS0_PIN_INDEX] = SDCard_SPIM_RTS_SS0_HSIOM_SEL_SPI;
                pinsDm   [SDCard_SPIM_RTS_SS0_PIN_INDEX] = SDCard_SPIM_PIN_DM_STRONG;
                pinsInBuf |= SDCard_SPIM_RTS_SS0_PIN_MASK;
            #endif /* (SDCard_SPIM_RTS_SS0_PIN) */
            ...
            #if (SDCard_SPIM_RX_SDA_MOSI_PIN)
                pinsInBuf |= SDCard_SPIM_RX_SDA_MOSI_PIN_MASK;
            #elif (SDCard_SPIM_RX_WAKE_SDA_MOSI_PIN)
                pinsInBuf |= SDCard_SPIM_RX_WAKE_SDA_MOSI_PIN_MASK;
            #else
            #endif /* (SDCard_SPIM_RX_SDA_MOSI_PIN) */

            #if (SDCard_SPIM_CTS_SCLK_PIN)
                pinsInBuf |= SDCard_SPIM_CTS_SCLK_PIN_MASK;
            #endif /* (SDCard_SPIM_CTS_SCLK_PIN) */
            }
        }
        else /* UART */
        {
            if (SDCard_SPIM_UART_MODE_SMARTCARD == subMode)
            {
                /* SmartCard */
            #if (SDCard_SPIM_TX_SCL_MISO_PIN)
                hsiomSel[SDCard_SPIM_TX_SCL_MISO_PIN_INDEX] = SDCard_SPIM_TX_SCL_MISO_HSIOM_SEL_UART;
                pinsDm  [SDCard_SPIM_TX_SCL_MISO_PIN_INDEX] = SDCard_SPIM_PIN_DM_OD_LO;
            #endif /* (SDCard_SPIM_TX_SCL_MISO_PIN) */
            }
            else /* Standard or IrDA */
            {
                ...
                {
                #if (SDCard_SPIM_RX_SDA_MOSI_PIN)
                    hsiomSel[SDCard_SPIM_RX_SDA_MOSI_PIN_INDEX] = SDCard_SPIM_RX_SDA_MOSI_HSIOM_SEL_UART;
                    pinsDm  [SDCard_SPIM_RX_SDA_MOSI_PIN_INDEX] = SDCard_SPIM_PIN_DM_DIG_HIZ;
                #elif (SDCard_SPIM_RX_WAKE_SDA_MOSI_PIN)
                    hsiomSel[SDCard_SPIM_RX_WAKE_SDA_MOSI_PIN_INDEX] = SDCard_SPIM_RX_WAKE_SDA_MOSI_HSIOM_SEL_UART;
                    pinsDm  [SDCard_SPIM_RX_WAKE_SDA_MOSI_PIN_INDEX] = SDCard_SPIM_PIN_DM_DIG_HIZ;
                #else
                #endif /* (SDCard_SPIM_RX_SDA_MOSI_PIN) */
                }
                ...
                if (SDCard_SPIM_UART_MODE_STD == subMode)
                {
                    ...
                    #if (SDCard_SPIM_CTS_SCLK_PIN)
                        hsiomSel[SDCard_SPIM_CTS_SCLK_PIN_INDEX] = SDCard_SPIM_CTS_SCLK_HSIOM_SEL_UART;
                        pinsDm  [SDCard_SPIM_CTS_SCLK_PIN_INDEX] = SDCard_SPIM_PIN_DM_DIG_HIZ;
                    #endif /* (SDCard_SPIM_CTS_SCLK_PIN) */
                    }
                    ...
                    #if (SDCard_SPIM_RTS_SS0_PIN)
                        hsiomSel[SDCard_SPIM_RTS_SS0_PIN_INDEX] = SDCard_SPIM_RTS_SS0_HSIOM_SEL_UART;
                        pinsDm  [SDCard_SPIM_RTS_SS0_PIN_INDEX] = SDCard_SPIM_PIN_DM_STRONG;
    ...
    /* Note: the DR register settings do not effect the pin output if HSIOM is other than GPIO */
        ...
        SDCard_SPIM_SET_HSIOM_SEL(SDCard_SPIM_RX_SDA_MOSI_HSIOM_REG,
                                       ...
                                       SDCard_SPIM_RX_SDA_MOSI_HSIOM_POS,
                                        hsiomSel[SDCard_SPIM_RX_SDA_MOSI_PIN_INDEX]);

        SDCard_SPIM_uart_rx_i2c_sda_spi_mosi_SetDriveMode((uint8) pinsDm[SDCard_SPIM_RX_SDA_MOSI_PIN_INDEX]);

        #if (!SDCard_SPIM_CY_SCBIP_V1)
            SDCard_SPIM_SET_INP_DIS(SDCard_SPIM_uart_rx_i2c_sda_spi_mosi_INP_DIS,
                                         SDCard_SPIM_uart_rx_i2c_sda_spi_mosi_MASK,
                                         (0u != (pinsInBuf & SDCard_SPIM_RX_SDA_MOSI_PIN_MASK)));
        #endif /* (!SDCard_SPIM_CY_SCBIP_V1) */
        ...
        SDCard_SPIM_SET_HSIOM_SEL(SDCard_SPIM_RX_WAKE_SDA_MOSI_HSIOM_REG,
                                       ...
                                       hsiomSel[SDCard_SPIM_RX_WAKE_SDA_MOSI_PIN_INDEX]);

        SDCard_SPIM_uart_rx_wake_i2c_sda_spi_mosi_SetDriveMode((uint8)
                                                               pinsDm[SDCard_SPIM_RX_WAKE_SDA_MOSI_PIN_INDEX]);

        SDCard_SPIM_SET_INP_DIS(SDCard_SPIM_uart_rx_wake_i2c_sda_spi_mosi_INP_DIS,
                                     SDCard_SPIM_uart_rx_wake_i2c_sda_spi_mosi_MASK,
                                     (0u != (pinsInBuf & SDCard_SPIM_RX_WAKE_SDA_MOSI_PIN_MASK)));

         /* Set interrupt on falling edge */
        SDCard_SPIM_SET_INCFG_TYPE(SDCard_SPIM_RX_WAKE_SDA_MOSI_INTCFG_REG,
                                        SDCard_SPIM_RX_WAKE_SDA_MOSI_INTCFG_TYPE_MASK,
    ...
    #if (SDCard_SPIM_TX_SCL_MISO_PIN)
        SDCard_SPIM_SET_HSIOM_SEL(SDCard_SPIM_TX_SCL_MISO_HSIOM_REG,
                                       SDCard_SPIM_TX_SCL_MISO_HSIOM_MASK,
                                       SDCard_SPIM_TX_SCL_MISO_HSIOM_POS,
                                        hsiomSel[SDCard_SPIM_TX_SCL_MISO_PIN_INDEX]);

        SDCard_SPIM_uart_tx_i2c_scl_spi_miso_SetDriveMode((uint8) pinsDm[SDCard_SPIM_TX_SCL_MISO_PIN_INDEX]);

    #if (!SDCard_SPIM_CY_SCBIP_V1)
        SDCard_SPIM_SET_INP_DIS(SDCard_SPIM_uart_tx_i2c_scl_spi_miso_INP_DIS,
                                     SDCard_SPIM_uart_tx_i2c_scl_spi_miso_MASK,
                                    (0u != (pinsInBuf & SDCard_SPIM_TX_SCL_MISO_PIN_MASK)));
    #endif /* (!SDCard_SPIM_CY_SCBIP_V1) */
    #endif /* (SDCard_SPIM_RX_SDA_MOSI_PIN) */

    #if (SDCard_SPIM_CTS_SCLK_PIN)
        SDCard_SPIM_SET_HSIOM_SEL(SDCard_SPIM_CTS_SCLK_HSIOM_REG,
                                       SDCard_SPIM_CTS_SCLK_HSIOM_MASK,
                                       SDCard_SPIM_CTS_SCLK_HSIOM_POS,
                                       hsiomSel[SDCard_SPIM_CTS_SCLK_PIN_INDEX]);

        SDCard_SPIM_uart_cts_spi_sclk_SetDriveMode((uint8) pinsDm[SDCard_SPIM_CTS_SCLK_PIN_INDEX]);

        SDCard_SPIM_SET_INP_DIS(SDCard_SPIM_uart_cts_spi_sclk_INP_DIS,
                                     SDCard_SPIM_uart_cts_spi_sclk_MASK,
                                     (0u != (pinsInBuf & SDCard_SPIM_CTS_SCLK_PIN_MASK)));
    #endif /* (SDCard_SPIM_CTS_SCLK_PIN) */

    #if (SDCard_SPIM_RTS_SS0_PIN)
        SDCard_SPIM_SET_HSIOM_SEL(SDCard_SPIM_RTS_SS0_HSIOM_REG,
                                       SDCard_SPIM_RTS_SS0_HSIOM_MASK,
                                       SDCard_SPIM_RTS_SS0_HSIOM_POS,
                                       hsiomSel[SDCard_SPIM_RTS_SS0_PIN_INDEX]);

        SDCard_SPIM_uart_rts_spi_ss0_SetDriveMode((uint8) pinsDm[SDCard_SPIM_RTS_SS0_PIN_INDEX]);

        SDCard_SPIM_SET_INP_DIS(SDCard_SPIM_uart_rts_spi_ss0_INP_DIS,
                                     SDCard_SPIM_uart_rts_spi_ss0_MASK,
                                     (0u != (pinsInBuf & SDCard_SPIM_RTS_SS0_PIN_MASK)));
    #endif /* (SDCard_SPIM_RTS_SS0_PIN) */
